...
const useMetaMask = () => {
  ...
  const { address, connect, isConnected, signMessage } = useWalletConnector(
    ...
    {
      ...walletBaseConfig,
      baseUrl: "https://metamask.io",
      encryptionPublicKeyName: "meta_mask_encryption_public_key",
    }
  );
  ...
  const handleConnect = async () => {
    try {
      ...
      Toast.show({
        type: "success",
        text1: "Connected to MetaMask wallet",
      });
    } catch (error) {
      Toast.show({
        type: "error",
        text1: "Error connecting MetaMask wallet",
    ...
    try {
      ...
      try {
        await login({
          ...
          wallet: {
            walletClientType: "metamask", // meta mask
