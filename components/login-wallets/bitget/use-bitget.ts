...
const useBitget = () => {
  ...
  const { address, connect, isConnected, signMessage } = useWalletConnector(
    ...
    {
      ...walletBaseConfig,
      baseUrl: "https://bkcode.vip",
      encryptionPublicKeyName: "bitget_encryption_public_key",
    }
  );
  ...
  const handleConnect = async () => {
    try {
      ...
      Toast.show({
        type: "success",
        text1: "Connected to Bitget wallet",
      });
    } catch (error) {
      Toast.show({
        type: "error",
        text1: "Error connecting Bitget wallet",
    ...
    try {
      ...
      try {
        await login({
          ...
          wallet: {
            walletClientType: "bitget_wallet", // bitkeep
